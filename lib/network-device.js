...
class NetworkDevice {
  ...
  setStatus(newStatus) {
    ...
    if (newStatus !== this.status) {
      this.log('NetworkDevice "%s" (%s) went from status "%s" to "%s"', this.config.name, this.config.ip || 'unknown ip', getSymbolDescription(this.status), getSymbolDescription(newStatus));
  ...
  getOnline(callback) {
    this.debugLog('Got state request for "%s" (%s): %s', this.config.name, this.config.ip || 'unknown ip', getSymbolDescription(this.status));
